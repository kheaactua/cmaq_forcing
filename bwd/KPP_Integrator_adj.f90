...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
SUBROUTINE INTEGRATE_ADJ( NADJ, YU, LamU, RKI, CHEMSTEP, &
...
!     Note: Lambda (1:KPPNSP,j) contains sensitivities of
!           the j-th cost functional w.r.t. Y(1:KPPNSP), j=1...NADJ
...
!     (used only for full continuous adjoint)   
   REAL(kind=dp) :: ATOL_adj(KPPNSP,NADJ), RTOL_adj(KPPNSP,NADJ)
...
!~~~>   INPUT ARGUMENTS: 
!    
!-     Y(KPPNSP)    = vector of initial conditions (at T=Tstart)
!      NADJ       -> dimension of linearized system, 
!                   i.e. the number of sensitivity coefficients
!-     Lambda(KPPNSP,NADJ) -> vector of initial sensitivity conditions (at T=Tstart)
...
!~~~>     OUTPUT ARGUMENTS:  
!     
!-    Y(KPPNSP)    -> vector of final states (at T->Tend)
!-    Lambda(KPPNSP,NADJ) -> vector of final sensitivities (at T=Tend)
...
!
!    ICNTRL(2)   = 0: AbsTol, RelTol are KPPNSP-dimensional vectors
...
!~~~>  Arguments   
   ...
   REAL(kind=dp), INTENT(IN)    :: AbsTol_adj(KPPNSP,NADJ), RelTol_adj(KPPNSP,NADJ)
...
!   For Vector tolerances (ICNTRL(2) == 0) the code uses AbsTol(1:KPPNSP) and RelTol(1:KPPNSP)
   IF (ICNTRL(2) == 0) THEN
      VectorTol = .TRUE.
      UplimTol  = KPPNSP
   ELSE 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   END IF   
   ALLOCATE( chk_Y(KPPNSP*S,bufsize), STAT=i )
   IF (i/=0) THEN
      PRINT*,'Failed allocation of buffer Y'; STOP
   END IF   
   ALLOCATE( chk_K(KPPNSP*S,bufsize), STAT=i )
   IF (i/=0) THEN
      PRINT*,'Failed allocation of buffer K'; STOP
   END IF  
   IF (SaveLU) THEN 
!#ifdef FULL_ALGEBRA
!     ALLOCATE( chk_J(KPPNSP*KPPNSP,bufsize), STAT=i )
!#else
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   IF (i/=0) THEN
      PRINT*,'Failed allocation of buffer T'; STOP
   END IF   
   ALLOCATE( chk_Y(KPPNSP,bufsize), STAT=i )
   IF (i/=0) THEN
      PRINT*,'Failed allocation of buffer Y'; STOP
   END IF   
   ALLOCATE( chk_dY(KPPNSP,bufsize), STAT=i )
   IF (i/=0) THEN
      PRINT*,'Failed allocation of buffer dY'; STOP
   END IF   
   ALLOCATE( chk_d2Y(KPPNSP,bufsize), STAT=i )
   IF (i/=0) THEN
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   REAL(kind=dp) :: T, H, Ystage(KPPNSP*S), K(KPPNSP*S)
   INTEGER       :: P(KPPNSP)
!#ifdef FULL_ALGEBRA
!   REAL(kind=dp) :: E(KPPNSP,KPPNSP)
!#else       
...
!#endif
   ...
   chk_H( stack_ptr ) = H
   chk_T( stack_ptr ) = T
   !CALL WCOPY(KPPNSP*S,Ystage,1,chk_Y(1,stack_ptr),1)
   !CALL WCOPY(KPPNSP*S,K,1,chk_K(1,stack_ptr),1)
   chk_Y(1:KPPNSP*S,stack_ptr) = Ystage(1:KPPNSP*S)
   chk_K(1:KPPNSP*S,stack_ptr) = K(1:KPPNSP*S)
   IF (SaveLU) THEN 
!#ifdef FULL_ALGEBRA
!       chk_J(1:KPPNSP,1:KPPNSP,stack_ptr) = E(1:KPPNSP,1:KPPNSP)
!       chk_P(1:KPPNSP,stack_ptr)        = P(1:KPPNSP)
!#else       
...
!#endif
   ...
   H = chk_H( stack_ptr )
   T = chk_T( stack_ptr )
   !CALL WCOPY(KPPNSP*S,chk_Y(1,stack_ptr),1,Ystage,1)
   !CALL WCOPY(KPPNSP*S,chk_K(1,stack_ptr),1,K,1)
   Ystage(1:KPPNSP*S) = chk_Y(1:KPPNSP*S,stack_ptr)
   K(1:KPPNSP*S)      = chk_K(1:KPPNSP*S,stack_ptr)
   !CALL WCOPY(LU_NONZERO,chk_J(1,stack_ptr),1,Jcb,1)
   IF (SaveLU) THEN
!#ifdef FULL_ALGEBRA
!       E(1:KPPNSP,1:KPPNSP) = chk_J(1:KPPNSP,1:KPPNSP,stack_ptr)
!       P(1:KPPNSP)        = chk_P(1:KPPNSP,stack_ptr)
!#else       
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   chk_H( stack_ptr ) = H
   chk_T( stack_ptr ) = T
   !CALL WCOPY(KPPNSP,Y,1,chk_Y(1,stack_ptr),1)
   !CALL WCOPY(KPPNSP,dY,1,chk_dY(1,stack_ptr),1)
   !CALL WCOPY(KPPNSP,d2Y,1,chk_d2Y(1,stack_ptr),1)
   chk_Y(1:KPPNSP,stack_ptr)   = Y(1:KPPNSP)
   chk_dY(1:KPPNSP,stack_ptr)  = dY(1:KPPNSP)
   chk_d2Y(1:KPPNSP,stack_ptr) = d2Y(1:KPPNSP)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   H = chk_H( stack_ptr )
   T = chk_T( stack_ptr )
   !CALL WCOPY(KPPNSP,chk_Y(1,stack_ptr),1,Y,1)
   !CALL WCOPY(KPPNSP,chk_dY(1,stack_ptr),1,dY,1)
   !CALL WCOPY(KPPNSP,chk_d2Y(1,stack_ptr),1,d2Y,1)
   Y(1:KPPNSP)   = chk_Y(1:KPPNSP,stack_ptr)
   dY(1:KPPNSP)  = chk_dY(1:KPPNSP,stack_ptr)
   d2Y(1:KPPNSP) = chk_d2Y(1:KPPNSP,stack_ptr)
...
!#ifdef FULL_ALGEBRA    
!   REAL(kind=dp) :: Jac0(KPPNSP,KPPNSP),  Ghimj(KPPNSP,KPPNSP)
!#else
...
Stage: DO istage = 1, ros_S
      ...
      ! For the 1st istage the function has been computed previously
       IF ( istage == 1 ) THEN
         ...
         IF (AdjointType == Adj_discrete) THEN ! Save stage solution
            ! CALL WCOPY(KPPNSP,Y,1,Ystage(1),1)
            Ystage(1:KPPNSP) = Y(1:KPPNSP)
      ...
      ! save stage solution every time even if ynew is not updated
       IF ( ( istage > 1 ).AND.(AdjointType == Adj_discrete) ) THEN
         ! CALL WCOPY(KPPNSP,Ynew,1,Ystage(ioffset+1),1)
         Ystage(ioffset+1:ioffset+KPPNSP) = Ynew(1:KPPNSP)
       END IF   
...
!#ifdef FULL_ALGEBRA
!   REAL(kind=dp), DIMENSION(KPPNSP,KPPNSP)  :: Jac, dJdT, Ghimj
!#else
...
!#ifdef FULL_ALGEBRA
!     Ghimj(1:KPPNSP,1:KPPNSP) = -Ghimj(1:KPPNSP,1:KPPNSP)
!     DO i=1,KPPNSP
...
Stage: DO istage = ros_S, 1, -1
      ...
      !~~~> Compute V 
       ...
       DO m = 1,NADJ
!#ifdef FULL_ALGEBRA
!         V(istart:istart+KPPNSP-1,m) = MATMUL(TRANSPOSE(Jac),U(istart:istart+KPPNSP-1,m))
!#else
...
!#ifdef FULL_ALGEBRA
!   REAL(kind=dp), DIMENSION(KPPNSP,KPPNSP)  :: Jac0, Ghimj, Jac, dJdT
!#else
   REAL(kind=dp), DIMENSION(LU_NONZERO) :: Jac0, Ghimj, Jac, dJdT
!#endif   
   REAL(kind=dp) :: H, Hnew, HC, HG, Fac, Tau 
   REAL(kind=dp) :: Err, Yerr(KPPNSP,NADJ)
   INTEGER :: Pivot(KPPNSP), Direction, ioffset, j, istage, iadj
   LOGICAL :: RejectLastH, RejectMoreH, Singular
!~~~>  Local parameters
   REAL(kind=dp), PARAMETER :: ZERO = 0.0d0, ONE  = 1.0d0 
   REAL(kind=dp), PARAMETER :: DeltaMin = 1.0d-5
...
!      DO iadj = 1, NADJ
!#ifdef FULL_ALGEBRA
!        dFdT(1:KPPNSP,iadj) = MATMUL(TRANSPOSE(dJdT),Y(1:KPPNSP,iadj))
!#else
...
!~~~>  Ydot = -J^T*Y
!#ifdef FULL_ALGEBRA
!   Jac0(1:KPPNSP,1:KPPNSP) = -Jac0(1:KPPNSP,1:KPPNSP)
!#else
   CALL WSCAL(LU_NONZERO,(-ONE),Jac0,1)
!#endif
   DO iadj = 1, NADJ
!#ifdef FULL_ALGEBRA
!     Fcn0(1:KPPNSP,iadj) = MATMUL(TRANSPOSE(Jac0),Y(1:KPPNSP,iadj))
!#else
...
Stage: DO istage = 1, ros_S
      ...
      ! istage>1 and a new function evaluation is needed at the current istage
       ELSEIF ( ros_NewF(istage) ) THEN
         ...
         ISTATUS(Njac) = ISTATUS(Njac) + 1
!#ifdef FULL_ALGEBRA
!         Jac(1:KPPNSP,1:KPPNSP) = -Jac(1:KPPNSP,1:KPPNSP)
!#else
         CALL WSCAL(LU_NONZERO,(-ONE),Jac,1)
!#endif
         DO iadj = 1, NADJ
!#ifdef FULL_ALGEBRA
!             Fcn(1:KPPNSP,iadj) = MATMUL(TRANSPOSE(Jac),Ynew(1:KPPNSP,iadj))
!#else
...
TimeLoop: DO istack = stack_ptr,2,-1
   ...
   T = chk_T(istack)
   H = chk_H(istack-1)
   !CALL WCOPY(KPPNSP,chk_Y(1,istack),1,Y0,1)
   Y0(1:KPPNSP) = chk_Y(1:KPPNSP,istack)
...
!#ifdef FULL_ALGEBRA
!     Ghimj(1:KPPNSP,1:KPPNSP) = -Jac0(1:KPPNSP,1:KPPNSP)
!     DO i=1,KPPNSP
      ...
      ! istage>1 and a new function evaluation is needed at the current istage
       ELSEIF ( ros_NewF(istage) ) THEN
         ...
         CALL ros_Hermite3( chk_T(istack-1), chk_T(istack), Tau, &
             chk_Y(1:KPPNSP,istack-1), chk_Y(1:KPPNSP,istack),       &
             chk_dY(1:KPPNSP,istack-1), chk_dY(1:KPPNSP,istack), Y0 )
         CALL JacTemplate(Y0, RKI, Jac)
         ISTATUS(Njac) = ISTATUS(Njac) + 1
!#ifdef FULL_ALGEBRA
!         Jac(1:KPPNSP,1:KPPNSP) = -Jac(1:KPPNSP,1:KPPNSP)
...
!~~~> Arguments   
!   REAL(kind=dp), INTENT(IN) :: T, Roundoff, Y(KPPNSP)
!#ifdef FULL_ALGEBRA    
!   REAL(kind=dp), INTENT(IN)  :: Jac0(KPPNSP,KPPNSP) 
!   REAL(kind=dp), INTENT(OUT) :: dJdT(KPPNSP,KPPNSP)   
!#else
...
!   ISTATUS(Njac) = ISTATUS(Njac) + 1
!#ifdef FULL_ALGEBRA    
!   CALL WAXPY(KPPNSP*KPPNSP,(-ONE),Jac0,1,dJdT,1)
!   CALL WSCAL(KPPNSP*KPPNSP,(ONE/Delta),dJdT,1)
!#else
...
!~~~> Output arguments
!#ifdef FULL_ALGEBRA    
!   REAL(kind=dp), INTENT(OUT) :: Ghimj(KPPNSP,KPPNSP)
!#else
...
!#ifdef FULL_ALGEBRA    
!     CALL WCOPY(KPPNSP*KPPNSP,Jac0,1,Ghimj,1)
!     CALL WSCAL(KPPNSP*KPPNSP,(-ONE),Ghimj,1)
...
!~~~> Inout variables
!#ifdef FULL_ALGEBRA    
!   REAL(kind=dp), INTENT(INOUT) :: A(KPPNSP,KPPNSP)
!#else   
...
!#ifdef FULL_ALGEBRA    
!   CALL  DGETRF( KPPNSP, KPPNSP, A, KPPNSP, Pivot, ising )
!#else   
...
!#ifdef FULL_ALGEBRA    
!   REAL(kind=dp), INTENT(IN) :: A(KPPNSP,KPPNSP)
!#else   
...
!~~~> InOut variables
   ...
   CASE ('N')
!#ifdef FULL_ALGEBRA    
!     CALL DGETRS( 'N', KPPNSP , 1, A, KPPNSP, Pivot, b, KPPNSP, 0 )
!#else   
     CALL KppSolve( A, b )
!#endif
   CASE ('T')
!#ifdef FULL_ALGEBRA    
!     CALL DGETRS( 'T', KPPNSP , 1, A, KPPNSP, Pivot, b, KPPNSP, 0 )
!#else   
...
!   ELSE
   ...
   CALL ros_Hermite3( chk_T(i), chk_T(i+1), T, &
                chk_Y(1:KPPNSP,i),   chk_Y(1:KPPNSP,i+1),     &
                chk_dY(1:KPPNSP,i),  chk_dY(1:KPPNSP,i+1),    &
                Y )
...
!~~~> Input variables     
   REAL(kind=dp), INTENT(IN)  :: Y(KPPNSP)
   REAL(kind=dp) :: RKI(KPPNRCT)
!~~~> Output variables     
   REAL(kind=dp), INTENT(OUT) :: Ydot(KPPNSP)
...
!~~~> Output variables
!#ifdef FULL_ALGEBRA    
!    REAL(kind=dp) :: JV(LU_NONZERO), Jcb(KPPNSP,KPPNSP)
!#else
