...
/* $Header: /home/amir/carleton/amir/work/orig/cmaq.4.5.1/models/CCTM/src/par/par/distr_env.c,v 1.1.1.1 2006-03-15 20:43:51 sjr Exp $  */
        ...
        for (i=0; i<env_size; i++)
            { str_size = strlen(environ_ptr[i]);
              if ( (environ_ptr[i] != NULL)&&(avail_size > str_size) )
                 ...
                 {
                   printf ("your temp_buf in distr_env may not big enough to hold next envionmental pair \n");
                   exit (1);
                 }
            }
