...
def printVec(vec, cstr):
	...
	for i in range(0, len(vec)):
		v=vec[i]
		if v > 0:
			outs=outs+"%s%4.2f%s"%(red, v, bc.clear)
		else:
			outs=outs+"%4.2f"%(v)
		if i<len(vec)-1:
			outs=outs+" "
...
print "Yest: %s%s%s"%(bc.yesterday, ' '.join('%4.3f' % v for v in yesterday), bc.clear)
print "Toda: %s%s%s"%(bc.today, ' '.join('%4.3f' % v for v in today), bc.clear)
print "Tomo: %s%s%s"%(bc.tomorrow, ' '.join('%4.3f' % v for v in tomorrow), bc.clear)
print "\n"
